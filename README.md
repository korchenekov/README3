...
**Операторы функции разделения (mergeSort):**
- **Оператор сравнения:** `if (arr.length < 2)` - проверка базового случая
- **Оператор арифметический:** `int mid = arr.length / 2` - вычисление середины массива
- **Операторы выделения памяти:** `int[] left = new int[mid]` - создание левого подмассива
- **Операторы выделения памяти:** `int[] right = new int[arr.length - mid]` - создание правого подмассива
- **Операторы копирования:** `System.arraycopy(arr, 0, left, 0, mid)` - копирование в левый массив
- **Операторы копирования:** `System.arraycopy(arr, mid, right, 0, arr.length - mid)` - копирование в правый массив
- **Операторы вызова:** `mergeSort(left)` - рекурсивный вызов для левой части
- **Операторы вызова:** `mergeSort(right)` - рекурсивный вызов для правой части
- **Операторы вызова:** `merge(arr, left, right)` - вызов процедуры слияния
...
- **Операции сравнения:** n × log₂n сравнений элементов при слиянии
- **Операции копирования:** 2n × log₂n операций копирования элементов
- **Операции присваивания:** n × log₂n присваиваний в результирующий массив
- **Операции выделения памяти:** O(n) операций для временных массивов
- **Рекурсивные вызовы:** 2n - 1 вызовов функций за все время выполнения

детаный анализ операций:

Операции функции mergeSort():
- Оператор условия: if (arr.length < 2) - 1 сравнение на вызов
- Оператор присваивания: int mid = arr.length / 2 - 1 операция деления на вызов
- Операторы создания массивов: new int[mid], new int[arr.length - mid] - 2 операции выделения памяти
- Операторы копирования: System.arraycopy() - 2 операции копирования по n/2 элементов
- Операторы рекурсивного вызова: mergeSort(left), mergeSort(right) - 2 вызова функции
- Оператор вызова слияния: merge(arr, left, right) - 1 вызов функции слияния
...
- **Обоснование сложности:**
  - Количество итераций пропорционально индексу k, где F(k) ≥ n
  - Так как F(k) ≈ φ^k/√5, то k ≈ log_φ(n) + константа
  - Поскольку φ ≈ 1.618, log_φ(n) = log₂n / log₂φ ≈ 1.44 log₂n
  - Каждая итерация требует фиксированного количества операций сравнения и арифметики
  - Общее время: O(log n) сравнений и O(log n) арифметических операций
